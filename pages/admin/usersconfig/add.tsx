...
const AdminPanelIndex = ({permissions={}}: any) => {
  ...
  const {control, formState: {errors}, handleSubmit, getValues} = useForm<User>();
    ...
    return (
        <Layout perms={permissions}>
          ...
          <form onSubmit={handleSubmit(sendData)} style={{width: "100%", maxWidth: "550px", textAlign: "center", margin: "0 auto"}}>
            ...
            <Box>
              <Controller
                ...
                rules={{
                  ...
                  validate: val => val === getValues("password") ? undefined : "Hasła muszą być identyczne"
                }}
                render={({ field: { onChange, onBlur, value, ref, name } }) => (
                  <CTextField
                    fullWidth
            ...
            <CButton LinkComponent={Link} href="/admin/usersconfig" disabled={loading}>
              Wróć
            </CButton>
            <CLoadingButton disabled={loading} loading={loading} startIcon={<AddIcon/>} loadingPosition="start" type="submit">
              Dodaj użytkownika
            </CLoadingButton>
          </form>
        </Layout>
