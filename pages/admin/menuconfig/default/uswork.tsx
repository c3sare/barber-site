...
const UsworkConfig = ({permissions, data}: any) => {
    ...
    const handleAddUsWork = () => {
      if(image === null) return;
        ...
        .then(data => {
          if(!data.error)
            setWorks(prevState => [...prevState, {_id: data._id, image: data.image}]);
        ...
        <div style={{position: "relative"}}>
          ...
          >
            ...
            <img
              ...
              src={image instanceof Blob ? URL.createObjectURL(image) : '/images/vercel.svg'}
              alt={`Obraz`}
            />
            <IconButton
              disabled={loading}
          ...
          <Grid container>
            {works.map((item:any) => (
              <Grid lg={4} sm={6} md={6} item sx={{position: "relative"}} key={item._id}>
                <Image
          ...
          {loading &&
            <div style={{position: "absolute", top: "0", left: "0", height: "100%", width: "100%", display: "flex", justifyContent: "center", alignItems: "center"}}>
              <Loading/>
            </div>
          }
        </div>
...
export const getServerSideProps = withIronSessionSsr(
  async function getServerSideProps({ req }) {
    ...
    const data = await getData("uswork");
