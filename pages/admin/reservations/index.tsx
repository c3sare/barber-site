...
import { Box, Divider, FormControl, IconButton, InputLabel, List, ListItem, ListItemText, MenuItem, Select, SelectChangeEvent, Tooltip } from "@mui/material";
...
const AdminPanelReservations = ({permissions, workers}: any) => {
  ...
  useEffect(() => {
    setLoading(true);
      fetch(`/api/reservations/${currentWorker}/${currentDate.format("YYYY-MM-DD")}`)
      .then(res => res.json())
      .then(res => {
        setList(res);
      })
  ...
  const handleAddTime = () => {
    ...
    return (
        <Layout perms={permissions}>
          ...
          <form style={{margin: "0 auto", textAlign: "center"}}>
            ...
            <div>
              ...
              <div style={{display: "flex", alignItems: "center", justifyContent: "center"}}>
                ...
                />
                <CLoadingButton loading={loading} disabled={loading} startIcon={<AddIcon/>} loadingPosition="start" onClick={handleAddTime}>
                  Dodaj
                </CLoadingButton>
              </div>
            </div>
            ...
            <Box sx={{ width: '100%', bgcolor: 'rgb(45, 45, 45)', borderRadius: "5px", overflow: "hidden", textAlign: "center"}}>
              <List sx={{padding: "0"}}>
                {!loading && list.sort(sortByTime).map((item, i) => (
                      ...
                      secondaryAction={
                        <>
                          <Tooltip title="Usuń" placement="bottom">
                            <IconButton
                              onClick={() => setData({id: item.time, open: true, text: `Czy chcesz usunąć godzinę ${list.find(itemf => itemf._id === item._id)!.time} z dnia ${currentDate.format("YYYY-MM-DD")} u Pracownika - ${workers.find((item:any) => item._id === currentWorker).name}?`})}
                          ...
                          <Tooltip title="Edytuj" placement="bottom">
                            <IconButton LinkComponent={Link} sx={{margin: "0 5px", color: "white", boxShadow: "none"}} href={"/admin/reservations/"+currentWorker+"/"+currentDate.format("YYYY-MM-DD")+"/"+item.time}>
                              <SettingsIcon/>
                            </IconButton>
                          </Tooltip>
                        </>
          ...
          <DeleteDialog setState={setList} open={data} id="time" setOpen={setData} url={`/api/reservations/${currentWorker}/${currentDate.format("YYYY-MM-DD")}`}/>
        </Layout>
    )
}
