...
const AdminPanelIndex = ({permissions={}, name}: any) => {
    ...
    const {handleSubmit, control, formState: {errors}} = useForm<Worker>({defaultValues: {
        name
    }});
    ...
    return (
        <Layout perms={permissions}>
            <form onSubmit={handleSubmit(handleSendData)} style={{margin: "0 auto", maxWidth: "550px", textAlign: "center"}}>
                ...
                <CButton LinkComponent={Link} disabled={loading} href="/admin/workersconfig">Wróć</CButton>
                <CLoadingButton type="submit" loading={loading} startIcon={<SaveIcon/>} loadingPosition="start">Dodaj</CLoadingButton>
            </form>
        </Layout>
    )
}
...
export const getServerSideProps = withIronSessionSsr(
    async function getServerSideProps({ req, query }) {
      ...
      const worker = workers.find((item:any) => item._id === query.id);
  
      if (user?.isLoggedIn !== true || !user?.permissions?.workers || worker === undefined) {
        return {
          notFound: true,
        };
      }
