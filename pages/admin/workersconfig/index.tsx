  ...
  useEffect(() => {
    ...
    return (
        <Layout perms={permissions}>
                    ...
                    secondaryAction={
                      <>
                        <Tooltip title="Usuń" placement="bottom">
                          <IconButton
                            onClick={() => setData({id: item._id, open: true, text: "Czy chcesz usunąć wybranego pracownika? - "+workers.find(itemf => itemf._id === item._id)!.name})}
                        ...
                        <Tooltip title="Edytuj" placement="bottom">
                          <IconButton LinkComponent={Link} sx={{margin: "0 5px", color: "white", boxShadow: "none"}} href={"/admin/workersconfig/"+item._id}>
                            <SettingsIcon/>
                          </IconButton>
                        </Tooltip>
                      </>
          ...
          <DeleteDialog setState={setWorkers} open={data} setOpen={setData} url="/api/workers"/>
        </Layout>
    )
}
