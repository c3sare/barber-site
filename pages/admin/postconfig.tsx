...
const AdminPanelPostConfig = ({permissions, data}: any) => {
  ...
  const {control, formState: {errors}, handleSubmit} = useForm<MailConfig>();
  ...
  const handleSendData = (data:any) => {
    ...
    return (
        <Layout perms={permissions}>
          ...
          <form onSubmit={handleSubmit(handleSendData)} style={{width: "100%", maxWidth: "550px", margin: "0 auto", textAlign: "center"}}>
            <Box pb={1}>
              <Controller
                control={control}
                name="host"
...
export const getServerSideProps = withIronSessionSsr(
    async function getServerSideProps({ req }) {
      ...
      const data = await getDataOne("mailConfig");
